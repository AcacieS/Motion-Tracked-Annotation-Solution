...
def load_video(video_path_str):
    ...
    return frame0_pil, editor_init, slider_update, info
...
def run_track(video_path_str, start_frame_idx, editor_value, frames_to_propagate, max_internal_size):
    ...
    if len(overlay_frames_bgr) == 0:
        raise gr.Error("No frames processed. Check video path / frames_to_propagate.")
    ...
    status = f"Done. start_frame={start_frame_idx}, processed={current}, overlay ready."
    return None, overlay_mp4, status
